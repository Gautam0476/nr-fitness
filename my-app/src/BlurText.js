...
const BlurText = ({ text = '', delay = 200, className = '', animateBy = 'words', direction = 'top', threshold = 0.1, rootMargin = '0px', animationFrom, animationTo, easing = 'easeOut', onAnimationComplete, stepDuration = 0.35 }) => {
    ...
    <div ref={ref} className={`flex flex-wrap justify-center ${className}`}>
      {elements.map((segment, index) => (
        <motion.span
          ...
          animate={inView ? buildKeyframes(animationFrom || defaultFrom, animationTo || defaultTo) : (animationFrom || defaultFrom)}
          transition={{ duration: stepDuration * 2, delay: (index * delay) / 1000, ease: easing }}
